...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await admin.from('ratings').upsert(
    { id: crypto.randomUUID(), reservationId: id, stars, comment: comment ?? null, tipCents: tipCents ?? 0 },
    { onConflict: 'reservationId' },
  );
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (tipCents) await admin.from('reservations').update({ gratuityCents: tipCents }).eq('id', id);
