...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: reservation } = await admin.from('reservations').select('customerId, rating:ratings(stars)').eq('id', id).single();
  if (!reservation) return NextResponse.json({ error: 'not found' }, { status: 404 });

  await admin.from('reservations').update({ status: 'CLOSED' }).eq('id', id);
  ...
  if (stars && stars >= 4) {
    await admin.from('notifications').insert({
      ...
      reservationId: id, sentVia: ['IN_APP'],
    });
  }
