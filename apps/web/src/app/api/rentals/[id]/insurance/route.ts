...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await admin.from('rental_bookings').update(update).eq('id', params.id).eq('customerId', user.id);
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const admin = createAdminClient();
  const { data: rental } = await admin.from('rental_bookings').select('customerId').eq('id', params.id).single();
