...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  ...
  console.warn('[supabase] Missing SUPABASE_URL / SUPABASE_ANON_KEY — check apps/mobile/.env');
}

// Standard Supabase + Expo setup: AsyncStorage as the session storage
// adapter, autoRefreshToken on, detectSessionInUrl off (no browser URL on
// native). Same Supabase project + anon key as apps/web (RLS applies
// identically — a signed-in user only ever sees their own rows).
